...
async def cmd_call_activing_dic_mode(message):
  ...
  # registコマンド
  if (REGISTER_CMD in message.content):
  ...
  # del <単語>
  elif (DELETE_CMD == message.content[:4]) and (dic_mode_vars["del_status"] == False):
    cmd = message.content.split()
    del_word = cmd[1]
  ...
  # yes
  elif (YES_CMD in message.content) and (dic_mode_vars["regi_status"] == True):
  ...
  # no
  elif (NO_CMD in message.content) and (dic_mode_vars["regi_status"] == True):
    dic_mode_vars["regi_status"] = False
    await message.channel.send(f"再度<{REGISTER_CMD}>コマンドを実行して下さい")
    return

  # c-list
  elif message.content == C_LIST_CMD:
    with open(CUSTOM_EMOJI_DIC_DIR, "r", encoding="UTF-8") as f:
      tex = f.read()
      await message.channel.send("カスタム絵文字辞書-登録済み一覧 ※単語(半角):::読み方\n-------------------\n" + tex + "-------------------")

  # c-del
  elif message.content[:6] == C_DELETE_CMD:
  ...
  # regi custom_emoji
  elif (C_REGISTER_CMD in message.content):
    dic_mode_vars["c_regi_status"] = True
    cmd = message.content.split()
    dic_mode_vars["regi_word"] = cmd[1]
    dic_mode_vars["regi_reading"] = cmd[2]
    if PLAY_MP3_CMD in dic_mode_vars["regi_reading"]:
  ...
  # yes
  elif (YES_CMD in message.content) and (dic_mode_vars["c_regi_status"] == True):
    if PLAY_MP3_CMD in dic_mode_vars["regi_reading"]:
  ...
  # no
  elif (NO_CMD in message.content) and (dic_mode_vars["c_regi_status"] == True):
    dic_mode_vars["c_regi_status"] = False
    await message.channel.send(f"再度<{C_REGISTER_CMD}>コマンドを実行して下さい")
    return 

  # outコマンド
  elif message.content == FIN_DIC_MODE_CMD:
